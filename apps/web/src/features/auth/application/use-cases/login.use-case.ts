import { LoginInputSchema } from "@ai-padrao/contracts";
...
export class LoginUseCase {
  ...
  async execute(input: unknown): Promise<void> {
    ...
    if (!parsed.success) return this.navigation.login("invalid_input");

    const result = await this.api.login(parsed.data).catch((error: unknown) => {
      if (error instanceof AuthFlowError) return error;
      throw error;
    });

    if (result instanceof AuthFlowError) {
      return this.navigation.login(result.apiMessage ?? "login_failed");
    }
